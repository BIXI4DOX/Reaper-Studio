--[[
   ReaScript Name: Copy & paste selected notes at mouse cursor (Default).
   ...
   About:
     ...
     Offsets selected notes vertically around mouse cursor if multiple pitches are selected.
--]]
...
local function paste_notes_at_mouse()                                                                                   --
    ...
    local minP, maxP = nil, nil                                                                                         --
                                                                                                                        --
    for _, n in ipairs(copyData.notes) do                                                                               -- Determine anchor pitch: if multiple distinct pitches are selected, use the vertical center
        if not minP or n.pitch < minP then minP = n.pitch end                                                           --
        if not maxP or n.pitch > maxP then maxP = n.pitch end                                                           --
    end                                                                                                                 --
                                                                                                                        --
    if minP and maxP and minP ~= maxP then                                                                              -- multiple distinct pitches present in selection
        anchorPitch = math.floor(((minP + maxP) / 2) + 0.5)                                                             -- if multiple pitches present: use center between min and max (rounded)
    else                                                                                                                --
        for _, n in ipairs(copyData.notes) do                                                                           -- single-pitch selection: preserve original behaviour (prefer a note at anchor start, else first note)
            if n.startppq == copyData.anchor then                                                                       --
                anchorPitch = n.pitch                                                                                   --
                break                                                                                                   --
            end                                                                                                         --
        end                                                                                                             --
        if not anchorPitch and #copyData.notes > 0 then                                                                 --
